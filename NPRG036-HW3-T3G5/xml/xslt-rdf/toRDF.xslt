...
<xsl:stylesheet version="1.0" 
    ...
    <xsl:template match="Zvíře">
        zoo:<xsl:value-of select="jméno"/> a dbo:Animal ;
        	...
        	dcterms:type zoo:<xsl:value-of select="názevTypu"/> ;
    ...
    <xsl:template match="ZvířeTyp">
        zoo:<xsl:value-of select="@název"/> a ex:ZvířeTyp;<!-- TODO  -->
            skos:inScheme zoo:TypyZvířat ;
            skos:prefLabel "<xsl:value-of select="@název"/>"@cs;
            ex:lzeSpatřit zoo:<xsl:value-of select="Výběh/@název"/> ; 
            dcterms:description "<xsl:value-of select="popis"/>"@cs ;
    ...
    <xsl:template match="Uklízeč">
        zoo:<xsl:value-of select="@název"/>  a ex:Uklízeč ;
            foaf:<xsl:value-of select="givenName"/> "Petr"@cs ;
            foaf:<xsl:value-of select="familyName"/> "Novotný"@cs ;
            gr:hasPriceSpecification zoo:<xsl:value-of select="plat/@název"/> ;
            vcard:hasTelephone <xsl:text disable-output-escaping="yes">&lt;</xsl:text>"tel:+420111222333"<xsl:text disable-output-escaping="yes">&gt;</xsl:text>  .
